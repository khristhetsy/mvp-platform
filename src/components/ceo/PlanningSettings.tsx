  ...
  return (
    <div style={{ maxWidth: 760 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
        ...
        <button type="button" onClick={suggest} disabled={suggesting} style={{ marginLeft: "auto", fontSize: 12, fontWeight: 600, color: royal, background: "#EEF3FC", border: "none", borderRadius: 8, padding: "8px 13px", cursor: suggesting ? "default" : "pointer", opacity: suggesting ? 0.7 : 1 }}>{suggesting ? "Thinking…" : (<><i className="ti ti-sparkles" aria-hidden="true" /> Suggest goals</>)}</button>
        <button type="button" onClick={() => setAdding((v) => !v)} style={{ fontSize: 12, fontWeight: 600, color: "#fff", background: navy, border: "none", borderRadius: 8, padding: "8px 13px", cursor: "pointer" }}>{adding ? "Cancel" : "+ New goal"}</button>
      </div>
      ...
      {suggestions && suggestions.length > 0 && (
        <div style={{ background: "#F7FAFF", border: "1px solid #DCE7FA", borderRadius: 12, padding: 12, marginBottom: 14 }}>
          <div style={{ display: "flex", alignItems: "center", marginBottom: 8 }}>
            ...
            <button type="button" onClick={() => setSuggestions(null)} style={{ marginLeft: "auto", background: "none", border: "none", fontSize: 15, color: "#98A2B3", cursor: "pointer", lineHeight: 1 }} aria-label="Dismiss"><i className="ti ti-x" aria-hidden="true" /></button>
          </div>
          {suggestions.map((g, i) => (
            <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "9px 0", borderTop: i ? "1px solid #E4ECF9" : "none" }}>
              ...
              <button type="button" onClick={() => addSuggested(g)} style={{ fontSize: 11.5, fontWeight: 600, color: "#fff", background: royal, border: "none", borderRadius: 7, padding: "6px 12px", cursor: "pointer", whiteSpace: "nowrap" }}>+ Add</button>
            </div>
          ))}
        </div>
      )}
      ...
      {adding && (
        <div style={{ background: "#F6F8FB", borderRadius: 10, padding: 12, marginBottom: 12, display: "grid", gridTemplateColumns: "2fr 1fr 1fr", gap: 8 }}>
          ...
          <button type="button" onClick={add} disabled={busy || !draft.title.trim()} style={{ fontSize: 12, fontWeight: 600, color: "#fff", background: royal, border: "none", borderRadius: 8, padding: "8px 13px", cursor: "pointer" }}>{busy ? "Saving…" : "Save goal"}</button>
        </div>
      )}
              ...
              <div key={g.id} style={{ padding: "12px 16px", borderTop: "1px solid #F1F4F9" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                  ...
                  <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 6, fontSize: 12 }}>
                    ...
                    <button type="button" onClick={() => del(g.id)} style={{ fontSize: 11.5, color: "#D6455D", background: "#FCE9EC", border: "none", borderRadius: 7, padding: "4px 9px", cursor: "pointer" }}>Delete</button>
                  </div>
                </div>
      ...
      <div>
        ...
        <div style={{ background: "#fff", border: "1px solid #E4E8F0", borderRadius: 12, padding: 16 }}>
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            <button type="button" onClick={runSnapshots} disabled={running !== null} style={{ fontSize: 12.5, fontWeight: 600, color: "#fff", background: navy, border: "none", borderRadius: 8, padding: "9px 14px", cursor: running ? "default" : "pointer", opacity: running === "snap" ? 0.6 : 1 }}>{running === "snap" ? "Computing…" : "Recompute snapshots"}</button>
            <button type="button" onClick={runBriefing} disabled={running !== null} style={{ fontSize: 12.5, fontWeight: 600, color: "#fff", background: royal, border: "none", borderRadius: 8, padding: "9px 14px", cursor: running ? "default" : "pointer", opacity: running === "brief" ? 0.6 : 1 }}>{running === "brief" ? "Generating…" : "Generate AI brief now"}</button>
          </div>
...
function MeetingRow({ meeting, first, onSaved }: { meeting: CeoMeeting; first: boolean; onSaved: () => void }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 16px", borderTop: first ? "none" : "1px solid #F1F4F9", flexWrap: "wrap" }}>
      ...
      <button type="button" onClick={save} disabled={busy || !dirty} style={{ marginLeft: "auto", fontSize: 12, fontWeight: 600, color: "#fff", background: dirty ? royal : "#C9D6EC", border: "none", borderRadius: 8, padding: "7px 13px", cursor: dirty ? "pointer" : "default" }}>{busy ? "Saving…" : "Save"}</button>
    </div>
  );
}
...
function Toggle({ on, onChange }: { on: boolean; onChange: (v: boolean) => void }) {
  return (
    <button type="button" onClick={() => onChange(!on)} aria-pressed={on} style={{ width: 40, height: 22, borderRadius: 11, border: "none", background: on ? royal : "#CBD5E1", position: "relative", cursor: "pointer" }}>
