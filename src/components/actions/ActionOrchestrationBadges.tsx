...
export function ActionOrchestrationBadges({ action }: Readonly<{ action: NextBestAction }>) {
  ...
  return (
    <div className="flex flex-wrap gap-1">
      {hints.needsAttention ? <Badge label={t("needs_attention")} className="border-rose-200 bg-rose-50 text-rose-800" /> : null}
      {hints.overdue ? (
        <Badge
          label={t("overdue")}
          className={
      ...
      {hints.escalated ? <Badge label={t("escalated")} className="border-amber-300 bg-amber-100 text-amber-950" /> : null}
      {hints.blocked ? <Badge label={t("blocked")} className="border-orange-200 bg-orange-50 text-orange-900" /> : null}
      {hints.inactivity ? <Badge label={t("inactive")} className="border-slate-200 bg-slate-100 text-slate-700" /> : null}
      {hints.reminder && !hints.overdue ? <Badge label={t("reminder")} className="border-indigo-200 bg-indigo-50 text-indigo-800" /> : null}
    </div>
  );
}
