      ...
      <div style={{ width: "100%", maxWidth: 560, background: "#fff", borderRadius: 14, boxShadow: "0 20px 50px rgb(12 35 64 / 0.28)", overflow: "hidden" }}>
        ...
        <div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 14 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div><span style={label}>Block</span>
            ...
            <div><span style={label}>Cadence</span>
          ...
          <div><span style={label}>Role note (one line)</span>
            <input style={input} value={roleNote} onChange={(e) => setRoleNote(e.target.value)} placeholder="What this surface is, in one sentence" />
          </div>

          <div><span style={label}>Live count source (optional)</span>
            <input style={input} value={countSource} onChange={(e) => setCountSource(e.target.value)} placeholder="investors_pending_kyc · intro_requests_pending · events_pending_applications" />
          </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {steps.map((s, i) => (
                <div key={i} style={{ display: "flex", gap: 6, alignItems: "flex-start" }}>
                  ...
                  <textarea value={s} onChange={(e) => setSteps((arr) => arr.map((x, j) => (j === i ? e.target.value : x)))} rows={2} style={{ ...input, resize: "vertical" }} placeholder="Supports **bold** and `code`" />
          ...
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
              <span style={label}>Gates &amp; guardrails</span>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {flags.map((f, i) => (
                <div key={i} style={{ display: "flex", gap: 6 }}>
                  ...
                  <input value={f.label} onChange={(e) => setFlags((arr) => arr.map((x, j) => (j === i ? { ...x, label: e.target.value } : x)))} style={input} placeholder="Rule" />
        ...
        <div style={{ padding: "12px 16px", borderTop: "0.5px solid #eef1f5", display: "flex", justifyContent: "flex-end", gap: 8 }}>
          <button type="button" onClick={onClose} style={{ fontSize: 13, color: "#5f5e5a", background: "#fff", border: "1px solid #d7dce4", borderRadius: 8, padding: "8px 14px", cursor: "pointer" }}>Cancel</button>
