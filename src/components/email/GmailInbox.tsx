...
export function GmailInbox() {
  ...
  const selectFolder = useCallback((f: GmailFolder) => {
    ...
    setOnlyUnread(false); setOnlyAttachments(false); setDateRange("all");
  }, []);
    ...
    if (inThread) {
      ...
      try {
        const res = await fetch(`/api/integrations/google/gmail/threads/${composeContext.threadId}/reply`, {
          method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ body: draft.body, html: draft.html }),
        });
        const data = await res.json();
        if (!res.ok) throw new Error(data.error ?? "Reply failed.");
        closeCompose();
