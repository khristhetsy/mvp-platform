...
export function UserPermissionsManager() {
  ...
  const save = async () => {
    ...
    try {
      ...
      setStatus("Permissions updated. Changes take effect immediately.");
      await load();
    } catch (err) {
  ...
  const deactivate = async () => {
    ...
    try {
      ...
      setStatus("Internal access deactivated.");
      await load();
    } catch (err) {
          ...
          <section className="space-y-4">
            ...
            {!canEditSelected ? (
              <p className="text-xs text-amber-800">
                You cannot edit this user&apos;s permissions — they are at or above your internal level.
              </p>
            ) : null}
          </section>
        ) : (
          <div className="rounded-xl border border-dashed border-slate-200 bg-white p-8 text-sm text-slate-500">
            Select an internal user to manage permissions.
          </div>
        )}
      </div>
    </div>
