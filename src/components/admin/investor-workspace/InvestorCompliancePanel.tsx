...
export function InvestorCompliancePanel({
  ...
  compliance: AdminInvestorWorkspaceData["compliance"];
  profileId: string;
}>) {
  return (
    <WorkspacePanel title="Compliance & review" subtitle="Admin-safe summaries only">
      ...
      {compliance.adminFeedback ? (
        <div className="mb-4 rounded-lg border border-amber-100 bg-amber-50 p-3 text-sm text-amber-900">
          <p className="text-xs font-semibold uppercase tracking-wide text-amber-700">Latest admin feedback</p>
          <p className="mt-1">{compliance.adminFeedback}</p>
        </div>
      ) : null}

      {compliance.nextAction ? (
        <p className="mb-4 rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm text-slate-800">{compliance.nextAction}</p>
      ) : null}

      {compliance.recentEvents.length === 0 ? (
        <EmptyState title="No compliance events" description="No compliance events recorded for this investor." />
      ) : (
      ...
      <p className="mt-4 text-xs text-slate-500">
        Source: compliance_events, investor_profiles ·{" "}
        <Link href={buildInvestorFilteredHref("/admin/compliance", profileId)} className="font-medium text-indigo-600 hover:text-indigo-800">
