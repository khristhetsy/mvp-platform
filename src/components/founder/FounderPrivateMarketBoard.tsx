...
const OUTREACH_META: Record<OutreachStatus, { label: string; className: string; dot: string }> = {
  clicked: { label: "Clicked", className: "bg-teal-50 text-teal-700", dot: "bg-teal-500" },
        ...
        <div className="flex items-start justify-between gap-3 px-5 pt-5">
          ...
          <div className="flex items-start gap-3">
            <div className="text-right">
              <p className="text-[9px] font-bold uppercase tracking-wide text-slate-400">Investor score</p>
              {r.scoreRated && r.investorScore != null ? (
                <span className="mt-0.5 inline-flex items-center gap-1 rounded-full bg-[#EEF0FB] px-2 py-0.5">
                  <span className="text-[15px] font-bold text-[#4338CA]">{r.investorScore}</span>
                  {r.scoreTier ? <span className="text-[10px] text-slate-500">{r.scoreTier}</span> : null}
                </span>
              ) : <span className="mt-0.5 block text-[13px] font-semibold text-slate-400">New</span>}
            </div>
        ...
        {visible.length === 0 ? (
          ...
          >
            ...
            <div className="flex flex-col items-start gap-1 sm:items-center">
              ...
              {r.outreach === "queued" && r.paused ? (
                <span className="inline-flex items-center gap-1 font-mono text-[10px] text-amber-600">
                  <svg width="9" height="9" viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M8 5v14M16 5v14" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round"/></svg>
                  Paused
                </span>
