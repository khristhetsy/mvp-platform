    ...
    return (
      <div className="px-5 pb-6 pt-5">
        ...
        <div className="mt-2">
          ...
          <BRow name="Preparation complete" badge={a.readinessScore != null ? `${a.readinessScore}% complete` : "Not yet assessed"} variant={a.readinessScore != null && a.readinessScore >= 80 ? "success" : a.readinessScore != null && a.readinessScore >= 50 ? "medium" : "neutral"} />
        ...
        <AdviceBox lines={[
          ...
          a.readinessScore != null ? `Your Preparation completeness of ${a.readinessScore}% is the investor-facing quality signal. Aim for 80% to maximise first-meeting conversion rate.` : "Complete onboarding to unlock your first diligence report and Preparation completeness — the key signal institutional investors use to filter listings.",
        ]} />
      </div>
    );
  }
    ...
    return (
      <div className="px-5 pb-6 pt-5">
        ...
        <div className="mt-4 rounded-lg bg-slate-50 px-4 py-3 ring-1 ring-slate-100">
          <p className="mb-1 text-[11px] font-semibold text-slate-700">{t("what_this_means")}</p>
          <p className="text-xs leading-relaxed text-slate-600">
            {latestScore == null ? "No completion score yet. It's generated from diligence reports which require a complete data room and company profile." : latestScore >= 80 ? `At ${latestScore}% complete, you're in the top tier — institutional investors typically require 80%+ before taking a first meeting.` : latestScore >= 60 ? `At ${latestScore}% complete, you have the core narrative in place but are missing verification documents. Closing document gaps is the fastest path to improving this.` : `At ${latestScore}% complete, you have significant diligence gaps. Most institutional investors filter below 60%, so improving this is a prerequisite to marketplace success.`}
          </p>
        </div>
        <AdviceBox lines={[
    ...
    return (
      <div className="px-5 pb-6 pt-5">
        ...
        <div className="mt-4 rounded-lg bg-slate-50 px-4 py-3 ring-1 ring-slate-100">
          <p className="mb-1 text-[11px] font-semibold text-slate-700">{t("what_this_means")}</p>
          <p className="text-xs leading-relaxed text-slate-600">
            {a.remediation.active === 0 ? "No active remediation tasks. These tasks appear when diligence reports or document reviews identify specific gaps that need addressing before investor meetings." : `You have ${a.remediation.active} active remediation task${a.remediation.active === 1 ? "" : "s"} (${a.remediation.open} open, ${a.remediation.inProgress} in progress). Each task represents a specific investor-readiness gap — completing them directly improves your Preparation completeness.`}
          </p>
        </div>
        <AdviceBox lines={[
          ...
          a.remediation.completed > 0 ? `You've completed ${a.remediation.completed} remediation task${a.remediation.completed === 1 ? "" : "s"} — each one is a diligence risk you've eliminated. Request a new readiness report to see the score impact.` : a.remediation.active > 0 ? "Completing remediation tasks typically improves your Preparation completeness by 5–15 points per task, depending on severity. Start with the highest-impact ones." : "Staying ahead of remediation is a strong operational signal for institutional investors who conduct structured due diligence.",
