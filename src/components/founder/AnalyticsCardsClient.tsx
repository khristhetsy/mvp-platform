    ...
    return (
      <div className="px-5 pb-6 pt-5">
        ...
        <div className="mt-4 rounded-lg bg-slate-50 px-4 py-3 ring-1 ring-slate-100">
          <p className="mb-1 text-[11px] font-semibold text-slate-700">{t("what_this_means")}</p>
          <p className="text-xs leading-relaxed text-slate-600">
            {a.remediation.active === 0 ? "No active remediation tasks. These tasks appear when diligence reports or document reviews identify specific gaps that need addressing before investor meetings." : `You have ${a.remediation.active} active remediation task${a.remediation.active === 1 ? "" : "s"} (${a.remediation.open} open, ${a.remediation.inProgress} in progress). Each task represents a specific investor-readiness gap — completing them directly improves your completion.`}
          </p>
        </div>
        <AdviceBox lines={[
          ...
          a.remediation.completed > 0 ? `You've completed ${a.remediation.completed} remediation task${a.remediation.completed === 1 ? "" : "s"} — each one is a diligence risk you've eliminated. Request a new readiness report to see the score impact.` : a.remediation.active > 0 ? "Completing remediation tasks typically improves your completion by 5–15 points per task, depending on severity. Start with the highest-impact ones." : "Staying ahead of remediation is a strong operational signal for institutional investors who conduct structured due diligence.",
