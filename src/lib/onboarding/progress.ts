...
export function computeFounderOnboardingProgress(input: {
  ...
  // Exclude admin-gated and system-generated steps from the founder progress meter.
  ...
  // Both are tracked separately in the readiness pipeline action items.
  const FOUNDER_ACTIONABLE_STEP_IDS = ONBOARDING_STEP_IDS.filter(
    (id) => id !== "investor_readiness_review" && id !== "readiness_generated",
  );
