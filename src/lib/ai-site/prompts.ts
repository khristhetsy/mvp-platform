import { guardrailPreamble } from "./guardrails";
 ...
 *
 * The prompts are BUILT PER CALL from the active pricing catalogue: the preamble
 * quotes plan prices, so freezing it at module scope would have the assistant
 * quoting whatever the code constants said at deploy time.
 */

function promptsFor(catalog: PricingCatalog): Record<AiTask, string> {
  const GUARDRAIL_PREAMBLE = guardrailPreamble(catalog);
...
/** The prompt for one task, priced from the catalogue the caller loaded. */
export function systemPromptFor(task: AiTask, catalog: PricingCatalog = CODE_DEFAULT_PRICING): string {
  return promptsFor(catalog)[task];
}
