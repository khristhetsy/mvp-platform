// AI IR Analyst — read-only narrative commentary on an IR metric. Cache-to-table
// (ir_analytics_insights), guardrailed: language denylist + whitelisted action keys.
// Heuristic fallback when Claude isn't configured. Never mutates data.
import crypto from "crypto";
import { z } from "zod";
import { serviceRoleClientUntyped } from "@/lib/supabase/admin";
import { claudeComplete, CLAUDE_HAIKU, isClaudeConfigured } from "@/lib/claude";
import { loadIrMetric, type IrMetric } from "./metrics";

// eslint-disable-next-line @typescript-eslint/no-explicit-any
function db(): any { return serviceRoleClientUntyped(); }

const ACTION_WHITELIST = new Set([
  "open_investor_crm", "open_intro_requests", "open_deal_rooms", "open_spvs",
  "open_compliance", "open_companies", "open_billing", "open_diligence", "open_matching",
]);
const DENYLIST = [/guarantee/i, /guaranteed/i, /will fund/i, /certain to/i, /promised/i];

export interface IrInsight {
...
function heuristic(metric: IrMetric): IrInsight {
  ...
  return {
    ...
    narrative: `${metric.label} is ${metric.value} (${metric.delta}), ${dir} over the last periods.${metric.note ? " " + metric.note : ""}`,
    model: null, drivers: metric.drivers, suggested_actions: [], cached: false,
  };
}

/** Cached-or-generated insight for one IR metric. Immutable-ish cache keyed on the data hash. */
export async function getIrInsight(metricKey: string, opts: { force?: boolean; createdBy?: string } = {}): Promise<IrInsight> {
  const metric = await loadIrMetric(metricKey);
  ...
  if (!opts.force) {
    const { data } = await db().from("ir_analytics_insights")
  ...
  await db().from("ir_analytics_insights").insert({
