...
/**
 * The row shape and the date presets live in `feed-types.ts` so the client
 * component can import them without dragging `server-only` into the browser
 * bundle. Re-exported here so server callers keep one import path.
 */
export type {
  ActivityFeedItem,
  ActivityStageGroup,
