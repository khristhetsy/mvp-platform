    ...
    <div>
      ...
      <div style={{ background: "#fff", border: "0.5px solid #e2e6ed", borderRadius: 12, overflow: "hidden" }}>
        ...
        {loading ? (
          ...
          return (
            <Link key={c.id} href={`/admin/sales/contacts/${c.id}`} style={{ display: "grid", gridTemplateColumns: "1.6fr 1.4fr 1fr 90px 120px", padding: "11px 14px", borderTop: "0.5px solid #eef1f5", alignItems: "center", fontSize: 12.5, textDecoration: "none", color: "var(--foreground)" }}>
              ...
              <div style={{ textAlign: "right", color: "var(--muted-foreground)" }}><i className="ti ti-chevron-right" aria-hidden="true" /></div>
            </Link>
          );
        })}
      </div>
      <p style={{ fontSize: 11, color: "var(--muted-foreground)", margin: "10px 2px 0" }}>Contacts sync from Odoo plus manual adds and other sources. Showing up to 100 matches. Open a contact to view the profile and convert it into an opportunity.</p>
    </div>
  );
}
