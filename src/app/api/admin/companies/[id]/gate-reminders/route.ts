...
async function founderIdFor(id: string): Promise<string | null> {
  const { data } = await db().from("companies").select("founder_id").eq("id", id).maybeSingle();
...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }): Promise<Response> {
  ...
  const founderId = await founderIdFor(id);
  const gates = await getCompanyGateReminders(id, founderId);
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }): Promise<Response> {
  ...
  if (action === "send-now") {
    const r = await sendGateReminderNow(id, founderId, gateKey);
    return NextResponse.json(r, { status: r.ok ? 200 : 400 });
  }
  if (action === "pause" || action === "resume") {
    await setGateReminderPaused(id, founderId, gateKey, action === "pause");
    return NextResponse.json({ ok: true });
  }
  if (action === "schedule") {
    const sendAt = String((body as { sendAt?: unknown }).sendAt ?? "");
    const recurring = Boolean((body as { recurring?: unknown }).recurring);
    const r = await setGateReminderSchedule(id, founderId, gateKey, sendAt, recurring);
    return NextResponse.json(r, { status: r.ok ? 200 : 400 });
  }
