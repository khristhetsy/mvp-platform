...
import { marketingSurfaceIds } from "@/lib/playbook/marketing-nav";
import { upsertModule } from "@/lib/playbook/store";

export const dynamic = "force-dynamic";

// Upsert editorial content for one marketing module (mkt:* key). Admin-only;
// analyst (read role) gets 403. Reuses the shared playbook_* store.
...
export async function PATCH(req: Request): Promise<Response> {
  try {
    ...
    if (!marketingSurfaceIds().has(navId)) {
      return NextResponse.json({ error: "Unknown marketing module." }, { status: 404 });
    }
