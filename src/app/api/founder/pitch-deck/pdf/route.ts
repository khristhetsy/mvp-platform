...
export async function GET(): Promise<Response> {
  ...
  try {
    ...
    const buffer = await renderDeckPdf({ id: deck?.id ?? "", companyId: g.company.id, slides, theme: deck?.theme ?? "navy", status: deck?.status ?? "draft", shareToken: null, aiAssisted: false, generatedAt: null, finalizedAt: null, updatedAt: null }, { name: g.company.company_name });
