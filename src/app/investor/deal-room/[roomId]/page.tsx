      ...
      <div className="space-y-6">
        ...
        <section className="grid gap-6 xl:grid-cols-2">
          <WorkspacePanel title="Questions" subtitle={`${(questions ?? []).length} recent`}>
            <DealRoomQuestionsPanel roomId={roomId} viewerRole="investor" initialQuestions={questions ?? []} />
          </WorkspacePanel>
          <WorkspacePanel title="Document requests" subtitle={`${(docRequests ?? []).length} recent`}>
            <DealRoomDocRequestsPanel roomId={roomId} viewerRole="investor" initialRequests={docRequests ?? []} />
          </WorkspacePanel>
        </section>
