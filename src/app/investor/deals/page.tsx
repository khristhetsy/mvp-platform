...
export default async function InvestorDealsPage() {
  ...
  return (
    <AppShell role="INVESTOR" workspace="investor" profileName={profile.full_name ?? profile.email ?? "Investor"} profileSubtitle={t("deals")}>
      <div className="space-y-5">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">{t("released_diligence_packages_shared_with_you")}</p>
        </div>

        <div className="overflow-hidden rounded-xl border border-slate-200/80 bg-white shadow-[var(--shadow-panel)]">
          {deals.length === 0 ? (
            <p className="p-6 text-sm text-slate-500">{t("no_deals_have_been_shared_with_you_yet")}</p>
          ) : (
