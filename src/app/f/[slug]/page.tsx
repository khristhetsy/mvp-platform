...
// founder_id is used server-side only (to resolve the Investable Score) and is
// never rendered.
const PUBLIC_FIELDS = [
  "id",
  "company_name",
...
export default async function InvestorOnePagerPage({
  ...
  const investableScore = company.founder_id
    ? await loadInvestableScore(createServiceRoleClient(), company.founder_id)
    : null;
    ...
    <div style={{ minHeight: "100vh", background: "#F8F9FC", fontFamily: "system-ui, sans-serif" }}>
      ...
      <main style={{ maxWidth: 760, margin: "0 auto", padding: "40px 20px 80px" }}>
        ...
        <p style={{ textAlign: "center", fontSize: 11, color: "#9ca3af", marginTop: 28, lineHeight: 1.6 }}>
          This one-pager was generated by iCapOS and shared by the founding team. It does not constitute
          a solicitation or offer to invest. Information is provided for indicative purposes only. The Investable
          Score is a rules-based readiness signal — not a rating of the securities or investment advice.
        </p>
      </main>
    </div>
  );
