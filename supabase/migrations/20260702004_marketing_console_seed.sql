...
--
-- Re-runnable: on conflict do nothing. Uses dollar-quoting ($$) for all text so
-- there are zero single quotes to be mangled when pasted into the SQL editor.

insert into public.playbook_module (nav_id, block, sort_order, role_note, cadence, count_source) values
  ($$mkt:dashboard$$,$$open$$,10,$$Engagement funnel plus integration health for the day.$$,$$daily$$,null),
  ...
  ($$mkt:lead-pipeline$$,$$core$$,40,$$Mirrored leads from Odoo plus import hygiene.$$,$$daily$$,null),
...
join (values
  ($$mkt:dashboard$$,1,$$Review the engagement funnel (sends to opens to clicks to replies to signups) at the Marketing hub Dashboard.$$),
  ...
  ($$mkt:action-center$$,2,$$For each staged send choose approve (routes to the campaign gate), hold, or fix.$$),
  ($$mkt:reply-inbox$$,1,$$Open replies in the inbox and CRM messages, set a route for each and mark handled.$$),
  ($$mkt:reply-inbox$$,2,$$Any reply mentioning guarantees or placement routes to securities counsel - do not answer directly.$$),
  ...
  ($$mkt:campaigns$$,1,$$Build and QA the campaign, then split a large send across three batches.$$),
  ($$mkt:campaigns$$,2,$$Sending is blocked until compliance review is approved - the send action returns 409 otherwise.$$),
  ($$mkt:phone$$,1,$$Call only non-suppressed leads and log each attempt.$$),
  ($$mkt:phone$$,2,$$The cap is two attempts per lead - the task auto-closes at the cap.$$),
  ($$mkt:assets$$,1,$$Review assets whose freshness is over 30 days, then bump the version or mark reviewed.$$),
  ($$mkt:brand$$,1,$$Fix any files flagged by the CI brand scan - the approved palette is navy to royal-blue only.$$),
  ...
  ($$mkt:eod$$,1,$$Review the engagement rollup and tomorrow staged batches, stage tomorrow, and write a one-line log of open blockers.$$)
...
join (values
  ($$mkt:reply-inbox$$,$$guardrail$$,$$Guarantee or placement language routes to securities counsel$$),
  ($$mkt:lead-pipeline$$,$$hard_gate$$,$$A segment cannot schedule until its import is clean$$),
  ($$mkt:campaigns$$,$$hard_gate$$,$$Send blocked unless compliance review is approved$$),
  ($$mkt:phone$$,$$hard_gate$$,$$Two-attempt call cap (enforced in the database)$$),
  ($$mkt:phone$$,$$guardrail$$,$$Suppress-on-convert - converted or unsubscribed leads are excluded$$),
  ($$mkt:brand$$,$$guardrail$$,$$Navy to royal-blue only; deprecated hexes flagged by CI$$),
