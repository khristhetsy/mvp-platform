-- iCapOS — event registration fields become editable data
--
-- Run in the Supabase SQL editor. Safe to re-run: the seed is guarded by
-- WHERE NOT EXISTS. Verified against real Postgres by
-- src/lib/icfo-events/registration-field-sets.pg.test.ts, which also asserts
-- the seed matches the code constants key for key — so the first version
...
-- Verify: one active version, seeded with the four attendee types.
select version,
       is_active,
       jsonb_array_length(roles)  as roles,
       jsonb_array_length(common) as common_fields,
       (select count(*) from jsonb_object_keys(by_type)) as types
