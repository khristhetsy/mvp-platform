-- iCapOS — event presenter invitations & materials
--
-- Run in the Supabase SQL editor. Safe to re-run: every create is guarded and
-- the ALTER TYPE uses IF NOT EXISTS. Verified against real Postgres by
-- src/lib/icfo-events/invites.pg.test.ts before it got here.
...
-- Verify: the enum gained 'exhibitor' and the table exists.
select 'kinds' as check, string_agg(enumlabel, ', ' order by enumsortorder) as value
  from pg_enum e join pg_type t on t.oid = e.enumtypid
 where t.typname = 'speaker_application_kind'
union all
select 'invite columns', count(*)::text from information_schema.columns
 where table_name = 'event_presenter_invites'
union all
select 'presenter materials', count(*)::text from information_schema.columns
 where table_name = 'event_presenters'
   and column_name in ('video_url','deck_path','deck_filename','deck_bytes','materials_updated_at');
